...
团队规则写在 `.ai-pr-review.yml` 的 `custom_rules` 里。每条规则会在新增代码行上做字面匹配，命中后进入普通 findings、风险评分、SARIF 和 PR 评论。
...
当前 `custom_rules` 适合放团队明确禁止的简单模式，例如危险反序列化、内部禁用 API、敏感 SDK 调用。它不适合写复杂语义分析；复杂规则应该沉淀到内置规则引擎和评估集里。
